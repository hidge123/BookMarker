...
async function management(name) {
  ...
  try {
    ...
    pass(`${name}: UI edit writes immediately`);
    ...
    await page.locator(`[data-node-id="${fixtures.links[1].id}"]`).dragTo(page.locator(`[data-node-id="${fixtures.links[0].id}"]`));
    ...
    pass(`${name}: rendered layout without horizontal overflow`);
    if (name === 'Chrome') {
      ...
      const rendered = await page.locator('.bookmark-row').count(); assert(rendered < 40, `Rendered ${rendered} rows`);
      ...
      assert(searchMs < 2000, `Search took ${searchMs} ms`); await page.screenshot({ path: join(output, 'chrome-10000-search.png'), fullPage: true }); pass('Chrome: 10,000 bookmark virtual list and search', `rendered=${rendered}, search=${searchMs}ms, fixture=${fixtureMs}ms`);
      ...
      await page.getByRole('dialog').getByRole('button', { name: '取消', exact: true }).click(); pass('Chrome: 10,000 selection and batch move preview', `${selectionMs}ms`);
    }
